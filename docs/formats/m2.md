...
Bytecode layout:
```
[1d]{Register data source: 8 bits}{Device: 16 bits}
[1e]{00}{Device: 16 bits}{Register note or identifier source: 8 bits}{24 bits of padding}
[1f]{Register data source: 8 bits}{Device: 16 bits}{Register note or identifier source: 8 bits}{24 bits of padding}
```
Human readable form:
```
e[[deviceID]]: [MIDI2.0 command with register number in data field]
;or
e[[deviceID]]: [MIDI2.0 command with register number in note or index field]
;or
e[[deviceID]]: [MIDI2.0 command with register number in note or index, and the data field]
```

Sends a single 64bit MIDI2.0 command, with the data, note, and/or index data being sourced from one or two registers.

## Position marker

Bytecode layout:
```
[20]{Position ID: 24 bits}
```
Human readable form:
```
...
Bytecode layout:
```
[21]{group/channel: 8 bits}{device ID: 16 bits}{key/mode id: 8 bits}{amount: signed 8 bits/register num}{immediate/register toggle: 1 bit}{reset: 1 bit}{all channels except current: 1 bit}{unused: 13 bits}
```
Human readable form:
```
