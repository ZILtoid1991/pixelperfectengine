...
public class QM816 : AudioModule {
	...
	*/
	public struct Operator {
		...
		void calculateKSL(ubyte note) @nogc @safe pure nothrow {
			if (!(preset.opCtrl & OpCtrlFlags.FixedPitch) && preset.kslBegin < note) {
				...
				outKSL = (1.0 - (octaves * ((preset.kslAttenOut / ubyte.max) * 0.75)));
	...
	enum ChCtrlFlags {
		...
		ResMode			=	1<<13,	///Enables resonant mode if available for the given algorithm
		ResSrc			=	1<<14,	///Selects the source for the resonance if available for the given algorithm
	}
	/**
	...
	*/
	...
	///Output is directed there before filtering and resampling
	///Layout is: LRAB
	protected __m128[]			initBuffers;
	//protected __m128[]			resamplBuffers;
	///Dummy buffer
	...
	protected float				levelFF = 1.0 / 64;
	...
	*/
	public this() @trusted nothrow {
		...
		try {
			...
			buffer = integrateTriangularWave(buffer);
			waveformDataReceive(28, reinterpretCast!ubyte(buffer), f);		///Integrated Ramp wave
			buffer = generateTriangularWave(768);
			waveformDataReceive(24, reinterpretCast!ubyte(buffer), f);		///Morphed ramp
			buffer = integrateTriangularWave(buffer);
			waveformDataReceive(29, reinterpretCast!ubyte(buffer), f);		///Integrated Morphed ramp
			buffer = generateTriangularWave(512);
			waveformDataReceive(25, reinterpretCast!ubyte(buffer), f);		///Triangle wave
			buffer = integrateTriangularWave(buffer);
			waveformDataReceive(30, reinterpretCast!ubyte(buffer), f);		///Integrated triangle wave
			buffer = generateTriangularWave(256);
			waveformDataReceive(26, reinterpretCast!ubyte(buffer), f);		///Morphed saw
			buffer = integrateTriangularWave(buffer);
			waveformDataReceive(31, reinterpretCast!ubyte(buffer), f);		///Integrated Morphed saw
			buffer = generateTriangularWave(2);
			waveformDataReceive(27, reinterpretCast!ubyte(buffer), f);		///Saw wave
			buffer = integrateTriangularWave(buffer);
			waveformDataReceive(32, reinterpretCast!ubyte(buffer), f);		///Integrated Saw wave
		} catch (Exception e) {
...
//
					...
					case MIDI1_0Cmd.PrgCh:	//Program change
						...
						presetNum[chOffset] = data0.program;
	...
	protected void keyOn(ubyte note, ubyte ch, float vel, float bend = float.nan) @nogc pure nothrow {
		...
		void hardReset() @safe @nogc pure nothrow {
			...
			operators[opOffset].eg.keyOn();
			operators[opOffset + 1].eg.keyOn();
			operators[opOffset].pos = 0;
			operators[opOffset + 1].pos = 0;
		}
		void softReset() @safe @nogc pure nothrow {
			channels[ch].eeg.keyOnNoReset();
			operators[opOffset].eg.keyOnNoReset();
			operators[opOffset + 1].eg.keyOnNoReset();
		}
		void hardResetCmb() @safe @nogc pure nothrow {
			channels[ch + 8].eeg.keyOn();
			operators[opOffset + 16].eg.keyOn();
			operators[opOffset + 17].eg.keyOn();
			operators[opOffset + 16].pos = 0;
			operators[opOffset + 17].pos = 0;
		}
		void softResetCmb() @safe @nogc pure nothrow {
			channels[ch + 8].eeg.keyOnNoReset();
			operators[opOffset + 16].eg.keyOnNoReset();
			operators[opOffset + 17].eg.keyOnNoReset();
		}
		...
		operators[opOffset].setFrequency(intSlmpRate, note, chCtrls[ch].pitchBend, channels[ch].preset.chnlTun);
		operators[opOffset].setEG(intSlmpRate, note, vel);
		operators[opOffset + 1].setFrequency(intSlmpRate, note, chCtrls[ch + 1].pitchBend, channels[ch].preset.chnlTun);
		operators[opOffset + 1].setEG(intSlmpRate, note, vel);
		if ((channels[ch].preset.chCtrl & ChCtrlFlags.ComboModeTest) && ch <= 7) {
			operators[opOffset + 16].setFrequency(intSlmpRate, note, chCtrls[ch].pitchBend, channels[ch].preset.chnlTun);
			operators[opOffset + 16].setEG(intSlmpRate, note, vel);
			operators[opOffset + 17].setFrequency(intSlmpRate, note, chCtrls[ch].pitchBend, channels[ch].preset.chnlTun);
			operators[opOffset + 17].setEG(intSlmpRate, note, vel);
	...
	protected void keyOff(ubyte note, ubyte ch, float vel, float bend = 0) @nogc pure nothrow {
		...
		operators[opOffset].keyOff(intSlmpRate);
		operators[opOffset + 1].keyOff(intSlmpRate);
		if ((channels[ch].preset.chCtrl & ChCtrlFlags.ComboModeTest) && ch <= 7) {
			channels[ch + 8].eeg.keyOff();
			operators[opOffset + 16].keyOff(intSlmpRate);
	...
	*/
	...
	public override void renderFrame(float*[] input, float*[] output) @nogc nothrow {
		...
		for (int i ; i < intBufSize ; i++) {
			const double x_0 = (wavetables[lfoWaveform[1] & byte.max][(aLFOPos>>22) & 0x0000_03FF] - short.min) * 
					(1 / cast(double)(ushort.max));
			const double x_1 = (wavetables[lfoWaveform[1] & byte.max][((aLFOPos>>22) + 1) & 0x0000_03FF] - short.min) * 
					(1 / cast(double)(ushort.max));
			const double p = (aLFOPos & 0x003F_FFFF) / cast(double)0x003F_FFFF;
			//aLFOBuf[i] = smoothstep(x_0, x_1, p);
			aLFOBuf[i] = (x_0 * (1 - p)) + (x_1 * p);
			aLFOPos += aLFORate;
		}
		//Generate pLFO out
		{
	...
	///Returns the sum of the level control values for use with the resonant mode
	pragma(inline, true)
	protected double updateOperator(ref Operator op, __m128 chCtrl, float outLev, float fbLev) @nogc @safe pure nothrow {
		...
		op.feedback = cast(int)((op.preset.opCtrl & OpCtrlFlags.FBMode ? (op.output<<12) : (op.output<<12) * egOut) * fbLev 
				* op.fbKSL * fbCtrl[0] * fbCtrl[1] * fbCtrl[2] * fbCtrl[3]);
		const double levelCtrl = outLev * egOut * op.outKSL * outCtrl[0] * outCtrl[1] * outCtrl[2];
	...
	protected final void updatePitchbend2Op(ref Operator op0, ref Operator op1, ref Channel ch, ref ChControllers chCtrl) 
		...
		if (!isClose(ch.preset.eegDetuneAm, 0.0, 0.1) || !isClose(ch.preset.pLFOlevel, 0.0, 0.01)
			...
			const float vibrAm = pLFOOut * ch.preset.pLFOlevel * 
					(ch.preset.chCtrl & ChCtrlFlags.MWToVibr ? chCtrl.modwheel : 1.0);
			op0.setFrequency(intSlmpRate, chCtrl.note, chCtrl.pitchBend + (ch.preset.eegDetuneAm * eegOut) + vibrAm, 
					ch.preset.chnlTun);
			op1.setFrequency(intSlmpRate, chCtrl.note, chCtrl.pitchBend + (ch.preset.eegDetuneAm * eegOut) + vibrAm, 
					ch.preset.chnlTun);
		}
	}
	...
	protected final void updatePitchbend4Op(ref Operator op0, ref Operator op1, ref Operator op2, ref Operator op3, 
		...
		if (!isClose(ch.preset.eegDetuneAm, 0.0, 0.1) || !isClose(ch.preset.pLFOlevel, 0.0, 0.01)
			...
			op1.setFrequency(intSlmpRate, chCtrl.note, chCtrl.pitchBend + (ch.preset.eegDetuneAm * eegOut) + vibrAm, 
					ch.preset.chnlTun);
			op2.setFrequency(intSlmpRate, chCtrl.note, chCtrl.pitchBend + (ch.preset.eegDetuneAm * eegOut) + vibrAm, 
					ch.preset.chnlTun);
			op3.setFrequency(intSlmpRate, chCtrl.note, chCtrl.pitchBend + (ch.preset.eegDetuneAm * eegOut) + vibrAm, 
					ch.preset.chnlTun);
		}
	}
	...
	static immutable string CHNL_UPDATE_CONSTS =
		q{
			...
			float lfopan = (channels[chNum].preset.chCtrl & ChCtrlFlags.LFOPan ? 1.0 : 0);
	...
	static immutable string CHNL_UPDATE_CONSTS_CYCL = 
		q{
			eegOut = channels[chNum].eeg.shp(channels[chNum].eeg.position == 
					ADSREnvelopGenerator.Stage.Attack ? channels[chNum].preset.shpAX : channels[chNum].preset.shpRX);
			eegToMast = __m128(eegOut), lfoToMast = __m128(aLFOBuf[i]);
	...
	protected void updateChannelM00(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			outSum = _mm_min_ps(_mm_max_ps(__m128(operators[opOffset + 1].output_0>>12), __m128(short.min)), __m128(short.max));
	...
	protected void updateChannelM01(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			outSum = _mm_min_ps(_mm_max_ps(__m128((outSum1 * resMode) + (outSum0 * (1 - resMode))),__m128(short.min)), __m128(short.max));
			mixin(CHNL_UPDATE_MIX);
			channels[chNum].eeg.advance();
		}
	}
	...
	protected void updateChannelM100(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			operators[opOffset + 16].feedback += 
				...
				(channels[chNum].preset.chCtrl & ChCtrlFlags.FBMode ? eegOut : 1));
			outSum = _mm_min_ps(
					_mm_max_ps(__m128(operators[opOffset + 1].output_0>>12), __m128(short.min)), __m128(short.max));
			mixin(CHNL_UPDATE_MIX0);
			channels[chNum].eeg.advance();
	...
	*/
	protected void updateChannelM101(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			//__m128 outSum = __m128((channels[chNum].preset.chCtrl & ChCtrlFlags.ResMode) ? outSum1 : outSum0);
			outSum = _mm_min_ps(
					_mm_max_ps(__m128((outSum1 * resMode) + (outSum0 * (1 - resMode))), __m128(short.min)), __m128(short.max));
			mixin(CHNL_UPDATE_MIX0);
			channels[chNum].eeg.advance();
			channels[chNum + 8].eeg.advance();
		}
	...
	protected void updateChannelM201(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			operators[opOffset + 16].feedback += 
				cast(int)(operators[opOffset].output_0 * channels[chNum].preset.globalFb * 
				(channels[chNum].preset.chCtrl & ChCtrlFlags.FBMode ? eegOut : 1));
			outSum = _mm_min_ps(_mm_max_ps(
					__m128((operators[opOffset + 1].output_0 + operators[opOffset].output_0)>>12), __m128(short.min)), 
					__m128(short.max));
			mixin(CHNL_UPDATE_MIX0);
			channels[chNum].eeg.advance();
			channels[chNum + 8].eeg.advance();
	...
	protected void updateChannelM211(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			operators[opOffset + 16].feedback += 
				...
				(channels[chNum].preset.chCtrl & ChCtrlFlags.FBMode ? eegOut : 1));
			outSum = _mm_min_ps(
					_mm_max_ps(__m128((operators[opOffset + 1].output_0 + operators[opOffset].output_0)>>12), __m128(short.min)), 
					__m128(short.max));
			mixin(CHNL_UPDATE_MIX0);
			channels[chNum].eeg.advance();
			channels[chNum + 8].eeg.advance();
	...
	protected void updateChannelM300(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			operators[opOffset + 16].feedback += 
				...
				(channels[chNum].preset.chCtrl & ChCtrlFlags.FBMode ? eegOut : 1));
			outSum = _mm_min_ps(
					_mm_max_ps(__m128((operators[opOffset + 1].output_0 + operators[opOffset + 17].output_0)>>12), __m128(short.min)), 
					__m128(short.max));
			mixin(CHNL_UPDATE_MIX0);
			channels[chNum].eeg.advance();
			channels[chNum + 8].eeg.advance();
	...
	*/
	protected void updateChannelM310(int chNum, size_t length) @nogc pure nothrow {
		...
		updatePitchbend4Op(operators[opOffset], operators[opOffset + 1], operators[opOffset + 16], operators[opOffset + 17], 
				channels[chNum], chCtrls[chNum]);
		const int resMode = (channels[chNum].preset.chCtrl & ChCtrlFlags.ResMode) ? 1 : 0;
		const int resSrc = (channels[chNum].preset.chCtrl & ChCtrlFlags.ResSrc) ? 1 : 0;
		for (size_t i ; i < length ; i++) {
			mixin(CHNL_UPDATE_CONSTS_CYCL);
			mixin(CHNL_UPDATE_CONSTS_CYCL0);
			updateOperator(operators[opOffset + 16], opCtrl2, opLevelCtrl0[0], opLevelCtrl0[1]);//S0
			...
			operators[opOffset].feedback += 
				cast(int)(operators[opOffset + 1].output_0 * channels[chNum].preset.globalFb * 
				(channels[chNum].preset.chCtrl & ChCtrlFlags.FBMode ? eegOut : 1));
			float outSum0 = (operators[opOffset + 1].output_0 + operators[opOffset + 17].output_0 + 
			...
			float outSum1 = ((cast(uint)(((resSrc * operators[opOffset + 1].output) + 
	...
	protected void updateChannelM301(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			operators[opOffset + 16].feedback += 
				...
				(channels[chNum].preset.chCtrl & ChCtrlFlags.FBMode ? eegOut : 1));
			outSum = _mm_min_ps(
					_mm_max_ps(__m128((operators[opOffset].output_0 + operators[opOffset + 1].output_0 + 
					operators[opOffset + 17].output_0)>>12), __m128(short.min)), __m128(short.max));
			mixin(CHNL_UPDATE_MIX0);
			channels[chNum].eeg.advance();
			channels[chNum + 8].eeg.advance();
		}
	...
	*/
	protected void updateChannelM311(int chNum, size_t length) @nogc pure nothrow {
		...
		for (size_t i ; i < length ; i++) {
			...
			float outSum1 = ((operators[opOffset].output_0 + operators[opOffset + 1].output_0)>>12) +
	...
	public override int writeParam_int(uint presetID, uint paramID, int value) nothrow {
		...
		switch (paramID) {
			...
			case hashCalc(`chf_FBNeg`):
				...
				return 0;
			case hashCalc(`chf_ResMode`):
				if (value == 0) {
					soundBank[bankNum][presetNum].channel.chCtrl &= ~ChCtrlFlags.ResMode;
				} else {
					soundBank[bankNum][presetNum].channel.chCtrl |= ChCtrlFlags.ResMode;
				}
				return 0;
			case hashCalc(`chf_ResSrc`):
				if (value == 0) {
					soundBank[bankNum][presetNum].channel.chCtrl &= ~ChCtrlFlags.ResSrc;
				} else {
					soundBank[bankNum][presetNum].channel.chCtrl |= ChCtrlFlags.ResSrc;
				}
				return 0;
	...
	public override MValue[] getParameters() nothrow {
		return [
			...
			MValue(MValueType.Float,`op0_Tune`), MValue(MValueType.Float,`op0_ShpA`),
			MValue(MValueType.Float,`op0_ShpR`), MValue(MValueType.Float,`op0_VelToLevel`), 
			...
			MValue(MValueType.Float,`op1_Tune`), MValue(MValueType.Float,`op1_ShpA`),
			MValue(MValueType.Float,`op1_ShpR`), MValue(MValueType.Float,`op1_VelToLevel`), 
			...
			MValue(MValueType.Boolean,`chf_ResMode`), MValue(MValueType.Boolean,`chf_ResSrc`),
	...
	public override int readParam_int(uint presetID, uint paramID) nothrow {
		...
		switch (paramID) {
			...
			case hashCalc(`chf_ResMode`):
				return (soundBank[bankNum][presetNum].channel.chCtrl & ChCtrlFlags.ResMode) ? 1 : 0;
			case hashCalc(`chf_ResSrc`):
				return (soundBank[bankNum][presetNum].channel.chCtrl & ChCtrlFlags.ResSrc) ? 1 : 0;
			//ch flags end
