...
public class QM816 : AudioModule {
	shared static this() {
		import pixelperfectengine.system.etc : max;
		for (int i ; i < 256 ; i++) {
			SINEWAVE_FRAGMENT[i] = cast(short)(sin((PI_2 / 256) * i) * short.max);
			LOGSINE_FRAGMENT[i] = cast(short)(max(log2(sin((PI_2 / 256) * i) + 2) * (short.max / 2), 0.0));
		}
		for (int i ; i < 128 ; i++) {
	...
	bit 5 = Stitches between normal (LO) and "logarithmic" (HI) sine wave fragment.
	*/
	public static short[1024] generateSinewave(ubyte[4] q) @nogc @safe pure nothrow {
		...
		for (int j ; j < 4 ; j++) {
			if (q[j] & 1) {
				...
				const short[256] fragment = q[j] & 0x20 ? LOGSINE_FRAGMENT : SINEWAVE_FRAGMENT;
				switch (q[j]>>1 & 3) {
					case 1:
						hMir *= 2;
						for (int i ; i < 256 ; i++) {
	...
	*/
	...
	/**
	Contains a quarter of a logarithmic sine wave, which is often used by other phase-modulation based algorithms.

	Formula: { y = log(sin(x*(pi/2)))+2 }
	*/
	public static immutable short[256] LOGSINE_FRAGMENT;
	/**
